...
const Experience = ({ sceneState, rotationSpeed, zoom }: { sceneState: 'CHAOS' | 'FORMED', rotationSpeed: number, zoom: number }) => {
  ...
  useFrame((_, delta) => {
    ...
    if (cameraRef.current) {
      // 映射 zoom 值 (0.2 - 1.2 左右) 到相机距离或 FOV
      // 这里采用修改相机 zoom 属性，需调用 updateProjectionMatrix
      ...
      if (gestureRecognizer && videoRef.current && canvasRef.current) {
        if (videoRef.current.videoWidth > 0) {
            ...
            if (results.landmarks.length > 0) {
              ...
              // 3. 放大缩小逻辑 (计算拇指尖 Tip: 4 和 食指尖 Tip: 8 的距离)
              ...
              // 映射距离 (通常在 0.05 到 0.4 之间) 到 Zoom 比例 (0.5 到 2.5)
...
export default function GrandTreeApp() {
  ...
  const [zoom, setZoom] = useState(1.0); // 初始缩放比例
