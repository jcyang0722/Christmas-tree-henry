...
const PhotoOrnaments = ({ state }: { state: 'CHAOS' | 'FORMED' }) => {
  ...
  const borderGeometry = useMemo(() => new THREE.PlaneGeometry(2, 1.8), []);
  const photoGeometry = useMemo(() => new THREE.PlaneGeometry(1.8, 1.8), []);
  ...
  const data = useMemo(() => {
    return new Array(count).fill(0).map((_, i) => {
      ...
      return { chaosPos, targetPos, scale: baseScale, weight, textureIndex: i % textures.length, borderColor, currentPos: chaosPos.clone(), chaosRotation, rotationSpeed, wobbleOffset: Math.random() * 10, wobbleSpeed: 0.5 + Math.random() * 0.5 };
    });
  }, [textures, count]);
  ...
  useFrame((stateObj, delta) => {
    ...
    groupRef.current.children.forEach((group, i) => {
      ...
      if (isFormed) {
         ...
         group.rotation.x += wobbleX; group.rotation.z += wobbleZ;
      } else {
         group.rotation.x += delta * objData.rotationSpeed.x; group.rotation.y += delta * objData.rotationSpeed.y; group.rotation.z += delta * objData.rotationSpeed.z;
      }
    });
  });
...
const ChristmasElements = ({ state }: { state: 'CHAOS' | 'FORMED' }) => {
  ...
  const data = useMemo(() => {
    return new Array(count).fill(0).map(() => {
      ...
      const currentRadius = (rBase * (1 - (y + (h/2)) / h)) * 0.95; const theta = Math.random() * Math.PI * 2;
      const targetPos = new THREE.Vector3(currentRadius * Math.cos(theta), y, currentRadius * Math.sin(theta));
...
const Experience = ({ sceneState, rotationSpeed, zoom }: { sceneState: 'CHAOS' | 'FORMED', rotationSpeed: number, zoom: number }) => {
  ...
  useFrame((_, delta) => {
    ...
    if (cameraRef.current) {
      cameraRef.current.zoom = THREE.MathUtils.damp(cameraRef.current.zoom, zoom, 2, delta);
      ...
      if (gestureRecognizer && videoRef.current && canvasRef.current) {
        if (videoRef.current.videoWidth > 0) {
            ...
            if (results.landmarks.length > 0) {
              ...
              const rotSpeed = (0.5 - hand[0].x) * 0.15;
              onMove(Math.abs(rotSpeed) > 0.01 ? rotSpeed : 0);
              ...
              const mappedZoom = THREE.MathUtils.mapLinear(distance, 0.05, 0.35, 1, 5);
              onZoom(THREE.MathUtils.clamp(mappedZoom, 0.5, 3.0));
            } else { 
              onMove(0); 
            }
        }
...
export default function GrandTreeApp() {
  ...
  const handleOpenGift = () => {
    ...
    // 延迟一秒自动汇聚树木，增强仪式感
    setTimeout(() => setSceneState('FORMED'), 1000);
  };
    ...
    <div style={{ width: '100vw', height: '100vh', backgroundColor: '#000', position: 'relative', overflow: 'hidden' }}>
      ...
      {!isStarted && (
        <div style={{
          ...
          backgroundImage: 'radial-gradient(circle, #0a2a12 0%, #000 100%)',
          ...
          <div 
            ...
            style={{
              cursor: 'pointer',
              textAlign: 'center',
              animation: 'box-bounce 2s infinite ease-in-out'
            }}
          >
            <div style={{ fontSize: '100px', marginBottom: '30px', animation: 'glow 2s infinite' }}>🎁</div>
            <h2 style={{ 
              color: '#FFD700', 
            ...
            <p style={{ color: '#fff', opacity: 0.5, fontSize: '12px', marginTop: '15px', letterSpacing: '2px' }}>
              A MAGICAL GIFT FOR HENRY
            </p>
          </div>
        </div>
      )}
      ...
      <div style={{
        ...
        opacity: sceneState === 'FORMED' && isStarted ? 1 : 0,
      ...
      <div style={{ width: '100%', height: '100%', position: 'absolute', top: 0, left: 0, zIndex: 1 }}>
        <Canvas dpr={[1, 2]} gl={{ toneMapping: THREE.ReinhardToneMapping }} shadows>
            <Experience sceneState={sceneState} rotationSpeed={rotationSpeed} zoom={zoom} />
        </Canvas>
      </div>
      ...
      <div style={{ position: 'absolute', bottom: '30px', left: '40px', color: '#888', zIndex: 10, fontFamily: 'sans-serif', userSelect: 'none' }}>
        ...
        <div>
          <p style={{ fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '4px' }}>Zoom Level</p>
